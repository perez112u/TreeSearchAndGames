...
public class UCS extends TreeSearch
{
     ...
     */
    public UCS(SearchProblem p, State initial_state) {
    ...
    public boolean solve() {
        ...
        frontier = new PriorityQueue<>(Comparator.comparingDouble(SearchNode::getCost));
        frontier.add(node);
        ...
        while(!frontier.isEmpty())
        {
            ...
            List<SearchNode> noeudsEnfants = actions.stream()
                    .map(action -> SearchNode.makeChildSearchNode(problem, finalNode, action))
                    .toList();
            ...
            for (SearchNode enfant : noeudsEnfants) {
                ...
                if (!explored.contains(stateEnfant) && !isInFrontier(stateEnfant)) {
                    ...
                    if (existingNode.getCost() > enfant.getCost()) {
